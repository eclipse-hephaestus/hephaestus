...
or `"drop"`/`"none"`). This file is scratch input for the command, not a
project artefact: write it to a temporary path outside the project, and never
inside `.pharaoh/`, which is the engine's own state directory. Step 4 tells
you when to delete it.
...
Once all three come back clean, delete the answers file you wrote in Step 3.
It has done its job: the result now lives in the project's `ubproject.toml`.
Keep it only while you are still iterating.
...
- Delete the answers file once Step 4 passes. The durable result of this
  skill is the process-graph block in `ubproject.toml`, and leaving scratch
  JSON behind in the user's repository is litter they did not ask for.
